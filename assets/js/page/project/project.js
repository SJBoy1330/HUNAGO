...
jQuery(document).ready(function(){
 ...
 var t = $('#kt_table_project').DataTable( {
        "columnDefs": [ {
            "searchable": false,
            "orderable": false,
            "targets": [0,1,2]
...
t.on( 'order.dt search.dt', function () {
    ...
    $("#kt_table_project").on('click', '.checkboxes', function(){
        var table = $('#kt_table_project').DataTable();
...
userdeleteModal.addEventListener('show.bs.modal', function (event){
    ...
    $("#nama").text(nama)
    $("#theidproject").val(id_project)

    $("#btn_delete_confirm").on('click', function(respdata){

        $.post(url + 'delete', {id_project: id_project}, function(respond){
...
modaldetail.addEventListener('show.bs.modal', function(event){
    ...
    var idproject = button.getAttribute('data-bs-id_project')
    var mode = button.getAttribute('data-bs-mode')

    var modalBodyInput = modaldetail.querySelector('.modal-body')

    $.post( url + 'detail', {id_project : idproject, mode: mode}, function(respdata){
