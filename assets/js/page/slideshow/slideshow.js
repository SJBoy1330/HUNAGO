...
jQuery(document).ready(function(){
 ...
 var t = $('#kt_table_slideshow').DataTable( {
        "columnDefs": [ {
            "searchable": false,
            "orderable": false,
            "targets": [0,1,2]
...
t.on( 'order.dt search.dt', function () {
    ...
    $("#kt_table_slideshow").on('click', '.checkboxes', function(){
        var table = $('#kt_table_slideshow').DataTable();
...
userdeleteModal.addEventListener('show.bs.modal', function (event){
    ...
    var id_slideshow = button.getAttribute('data-bs-id_slideshow')
    var judul = button.getAttribute('data-bs-judul')

    $("#judul").text(judul)
    $("#idslideshow").val(id_slideshow)

    $("#btn_delete_confirm").on('click', function(respdata){

        $.post(url + 'delete', {id_slideshow: id_slideshow}, function(respond){
            if (respond.status == 'OK'){
                ...
                Swal.fire({
                      title: 'Pesan Gagal!',
                      html: 'Gagal Menghapus Slideshow',
                      icon: 'error',
                      confirmButtonText: 'Tutup'
                    })
            }
