...
jQuery(document).ready(function(){
 ...
 var t = $('#kt_table_kategori_project').DataTable( {
        "columnDefs": [ {
            "searchable": false,
            "orderable": false,
            "targets": [0,1,2]
...
t.on( 'order.dt search.dt', function () {
    ...
    $("#kt_table_kategori_project").on('click', '.checkboxes', function(){
        var table = $('#kt_table_kategori_project').DataTable();
    ...
    $("#btn_delete_selected").on("click", function(){
        ...
        $("#jumlahkategori").empty();
        $("#jumlahkategori").text(jumlah);
    });
...
userdeleteModal.addEventListener('show.bs.modal', function (event){
    ...
    var id_kategori = button.getAttribute('data-bs-id_project_kategori')
    var nama = button.getAttribute('data-bs-nama')

    $("#nama").text(nama)
    $("#theidkategori").val(id_kategori)

    $("#btn_delete_confirm").on('click', function(respdata){

        $.post(url + 'delete', {id_project_kategori: id_kategori}, function(respond){
            if (respond.status == 'OK'){
                ...
                Swal.fire({
                      title: 'Pesan Gagal!',
                      html: 'Gagal Menghapus Kategori Blog',
                      icon: 'error',
                      confirmButtonText: 'Tutup'
                    })
            }
