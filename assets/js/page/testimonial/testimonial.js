...
jQuery(document).ready(function(){
 ...
 var t = $('#kt_table_testimonial').DataTable( {
        "columnDefs": [ {
            "searchable": false,
            "orderable": false,
            "targets": [0,1,2]
...
t.on( 'order.dt search.dt', function () {
    ...
    $("#kt_table_testimonial").on('click', '.checkboxes', function(){
        var table = $('#kt_table_testimonial').DataTable();
    ...
    $("#btn_delete_selected").on("click", function(){
        ...
        $("#jumlahtestimonial").empty();
        $("#jumlahtestimonial").text(jumlah);
    });
...
var userModal = document.getElementById('testimoniModal')
userModal.addEventListener('show.bs.modal', function (event) {
  // Button that triggered the modal
  var button = event.relatedTarget
  // Extract info from data-bs-* attributes
  var id_testimonial = button.getAttribute('data-bs-id_testimonial')
  // If necessary, you could initiate an AJAX request here
  // and then do the updating in a callback.
  //
  // Update the modal's content.

  $.post(url + 'detail', {id_testimonial: id_testimonial}, function(respdata){
...
userdeleteModal.addEventListener('show.bs.modal', function (event){
    ...
    $("#nama").text(nama)
    $("#id_testimonial").val(id_testimonial)

    $("#btn_delete_confirm").on('click', function(respdata){

        $.post(url + 'delete', {id_testimonial: id_testimonial}, function(respond){
            if (respond.status == 'OK'){
                ...
                Swal.fire({
                      title: 'Pesan Gagal!',
                      html: 'Gagal Menghapus Testimonial',
                      icon: 'error',
                      confirmButtonText: 'Tutup'
                    })
            }
