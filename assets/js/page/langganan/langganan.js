var gModul = 'langganan/';
var url = baseUrl + gModul;


jQuery(document).ready(function(){
    
 var t = $('#kt_table_langganan').DataTable( {
        "columnDefs": [ {
            "searchable": false,
            "orderable": false,
            "targets": [0,1,2]
...
t.on( 'order.dt search.dt', function () {
    ...
    $("#kt_table_langganan").on('click', '.checkboxes', function(){
        var table = $('#kt_table_langganan').DataTable();
    ...
    $("#btn_delete_selected").on("click", function(){
        ...
        $("#jumlahlangganan").empty();
        $("#jumlahlangganan").text(jumlah);
    });
...
userdeleteModal.addEventListener('show.bs.modal', function (event){
    ...
    var id_langganan = button.getAttribute('data-bs-id_langganan')
    var email = button.getAttribute('data-bs-email')

    $("#email").text(email)
    $("#id_langganan").val(id_langganan)

    $("#btn_delete_confirm").on('click', function(respdata){

        $.post(url + 'delete', {id_langganan: id_langganan}, function(respond){
            if (respond.status == 'OK'){
                ...
                Swal.fire({
                      title: 'Pesan Gagal!',
                      html: 'Gagal Menghapus Langganan',
                      icon: 'error',
                      confirmButtonText: 'Tutup'
                    })
            }
