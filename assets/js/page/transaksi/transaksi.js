...
jQuery(document).ready(function(){
 ...
 var t = $('#kt_table_transaksi').DataTable( {
        "columnDefs": [ {
            "searchable": false,
            "orderable": false,
            "targets": [0,1,2]
    ...
    $("#kt_table_transaksi").on('click', '.checkboxes', function(){
        var table = $('#kt_table_transaksi').DataTable();
    ...
    $("#btn_delete_selected").on("click", function(){
        ...
        $("#jumlahtransaksi").empty();
        $("#jumlahtransaksi").text(jumlah);
    });
...
userdeleteModal.addEventListener('show.bs.modal', function (event){
    ...
    var id_transaksi = button.getAttribute('data-bs-id_transaksi')
    ...
    $("#btn_delete_confirm").on('click', function(respdata){
        ...
        $.post(url + 'delete', {id_klien: id_klien}, function(respond){
