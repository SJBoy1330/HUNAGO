...
class Welcome extends MY_Admin
{
    ...
    public function single($id_video = NULL)
    {
        ...
        if ($tag) {
            ...
            $terkait = $this->video_tag_m->get_where_params(array('video.id_video !=' => $id_video), 'DISTINCT(video_tag.id_video),video.*', $p);
        } else {
            $terkait = NULL;
        }
        if ($this->id_user != NULL) {
    ...
    public function testyt(){
        ...
        // set url
        curl_setopt($ch, CURLOPT_URL, "https://www.youtube.com/watch?v=bBnb7lqURV4");
        ...
        $dokumen = htmlspecialchars($output);
        ...
        foreach($line as $strline){
            ...
            $judul =  str_replace(array("&lt;title&gt;", "&lt;/title&gt;"), "", $hasil[0]);
